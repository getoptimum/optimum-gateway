...
func (s *Server) handle(w http.ResponseWriter, r *http.Request) {
	...
	// no subscriber.
	connID, ok := s.limiter.acquire(subject)
	if !ok {
	...
	if err != nil {
		...
		s.limiter.release(subject, connID)
		return
	}

	sub := s.hub.Subscribe(s.cfg.BufferSize)
	go s.serve(conn, sub, subject, token, connID, mode == modeRaw)
}

func (s *Server) serve(conn *websocket.Conn, sub *streamhub.Subscription, subject, token string, connID uint64, raw bool) {
	...
	// Read pump: refreshes the idle deadline, forwards a refreshed token if the
	// consumer sends one, and closes done on any read error to unblock the
	// writer. Still read-only in every other sense.
	...
	defer ping.Stop()
	hb, hbC := heartbeatTicker(s.cfg.HeartbeatInterval)
	if hb != nil {
		defer hb.Stop()
	}
	ra, raC := reauthTicker(&s.cfg)
	if ra != nil {
		defer ra.Stop()
	}
	...
	for {
		...
		case t := <-refresh:
			// Verified on the writer, which owns token, so a rejected refresh
			// cannot change who this connection is authenticated as.
			if _, err := s.auth.Authenticate(t); err != nil {
				telemetry.RecordStreamAuthFailure()
				continue
			}
			token = t
		case <-raC:
			// Re-verifying the last presented token is what makes expiry
			// visible; the verifier checks exp, so nothing is tracked here.
			if _, rerr := s.auth.Authenticate(token); rerr != nil {
				telemetry.RecordStreamReauthFailure()
				if s.cfg.ReauthMode == ReauthEnforce {
					// Say why. A consumer holding a stream for weeks needs to
					// tell an auth cut from a dead network, and this is the
					// only close frame the transport sends.
					_ = conn.WriteControl(websocket.CloseMessage,
						websocket.FormatCloseMessage(websocket.ClosePolicyViolation, "token expired, refresh required"),
						time.Now().Add(writeWait))
					s.log.Error("closing consumer stream, token no longer verifies (enforce mode)", rerr,
				...
				if !reauthFailing {
					reauthFailing = true
					s.log.Error("consumer stream token no longer verifies, keeping stream (observe mode)", rerr,
						logger.WithString("subject", subject))
				}
			} else {
				reauthFailing = false
...
// heartbeatFrame proves the feed is alive during a quiet stretch and says how
// far behind it is. A data frame rather than a control ping on purpose:
// browsers cannot observe WebSocket ping/pong at all, so a control frame is
// invisible to exactly the consumers most likely to sit idle.
