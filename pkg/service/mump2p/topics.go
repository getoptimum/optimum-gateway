...
func (n *Node) SubscribeTopic(topicName string) error {
	...
	// The router only recodes and forwards for topics it tracks, so a joined topic
	// that is never tracked receives symbols but relays none of them.
	n.router.TrackTopic(topicName)
