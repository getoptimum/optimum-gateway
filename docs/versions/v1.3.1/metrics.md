...
## Authentication (API key / join key / JWT)

| Metric                                         | Type    | Labels | Description                                                                                                                            |
| ---------------------------------------------- | ------- | ------ | -------------------------------------------------------------------------------------------------------------------------------------- |
| `mump2p_gateway_auth_enrollment_total`         | Counter | result | Join-key enrollment at startup (result: `success` = new enroll persisted, `reused` = loaded existing credential). Repeated `success` across restarts means the credential directory is not persisting |
| `mump2p_gateway_auth_token_mint_total`         | Counter | result | JWT mint outcomes (result: success, unknown_key, revoked, suspended, forbidden, bad_status, network_error, empty_token, verify_failed, assertion_failed) |
